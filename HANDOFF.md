...
- Pages Settings now has a 128-character top-level literal-first search across all 12 current Settings sections, with its own anchored full JavaScript regex builder, effective-value aliases, honest no-match state, and command-palette focus destination. Nested project/history/notification/changelog builders remain independent; runtime filtering and focus remain unverified.
- Pages tab navigation now measures the rendered strip at every dock edge and in the narrow horizontal fallback. When the six existing destinations do not fit, a bounded local all-tabs surface provides plain-text-first discovery and an independent anchored JavaScript regex builder. Reordering, pinning, groups, bulk close, runtime interaction, and visual evidence remain outside this lane and unverified.
