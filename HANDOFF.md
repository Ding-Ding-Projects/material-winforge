...
- Archive path: `C:\Users\cntow\OneDrive\OakKayBackups\material-winforge\zips\material-winforge-20260918T184044Z.7z`.
- Archive size: `2,705,831` bytes. The archive contains `316` entries, including `181` Git-administrative-directory entries.
...
- Added `docs/application/destructive-confirmation.md`, the application-doc index entry, the universal inventory update, and the Unreleased changelog entry. Public documentation keeps ordinary terminology.
- Tests, lint, reviews, packaged runtime interaction, accessibility review, and captures were not run. The desktop build and publication preflights are the remaining local evidence before integration.
...
The desktop Settings surface now includes four bundled logo presets and a local PNG/JPEG picker. MIME and magic-byte checks, the 512 KiB file bound, decoded 2048×2048 pixel bound, and the 360,000-character local cache bound fail closed while retaining the previous valid mark. The schema-2 editor adds keyboard-accessible focal-point X/Y controls, bounded zoom, contain/cover/fill fit, transparent/surface/primary-container background treatment, and locally validated 26/64/128px PNG derivatives. Conversion is transactional: a failed derivative build restores the previous source, crop, and outputs. Preset/custom selection persists locally and updates the frameless title bar and Settings preview live. Product/package identity, installer identity, update feed, and application-data location remain fixed.
