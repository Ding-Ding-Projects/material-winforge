...
function normalizePreferences(value: unknown): Preferences | null {
  ...
  return { ...DEFAULTS, ...v, ...effective, schemaVersion: 1, personalVocabulary, logoPreset: v.logoPreset ?? "forge", customLogo: v.customLogo ?? null, settingsOwnership } as Preferences;
}
...
export default function SiteShell({ assetBase }: { assetBase: string }) {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") { setPaletteOpen(false); setPaletteBuilderOpen(false); setBuilderOpen(false); setNotificationOpen(false); setNotificationBuilderOpen(false); setSettingsHistoryOpen(false); setSettingsHistoryBuilderOpen(false); setSettingsRestoreId(null); setResetConfirmOpen(false); resetCommitted.current = false; setResetKeySettings(false); setResetKeyProjects(false); setResetSlider(0); setResetComplete(false); setTimeout(() => resetOrigin.current?.focus(), 0); }
    };
  ...
  commands.push({ id: "app-logo", label: dual("App logo customization", "應用程式標誌自訂", language), detail: dual("Open shipped presets and local custom image controls", "開啟內置預設同本機自訂圖片控制", language), action: () => selectTab("settings", "site-logo-settings") });
  commands.push({ id: "app-logo-upload", label: dual("Upload custom app logo", "上載自訂應用程式標誌", language), detail: dual("Choose or replace a bounded local PNG or JPEG", "揀選或取代有限度本機 PNG 或 JPEG", language), action: () => { selectTab("settings", "site-logo-file"); setTimeout(() => document.getElementById("site-logo-file")?.click(), 60); } });
  ...
  commands.push({ id: "tab-docking", label: dual("Tab position", "分頁位置", language), detail: dual("Choose left, right, top, or bottom docking", "揀左、右、頂或者底部停靠", language), action: () => selectTab("settings", "tab-docking-setting") });
  ...
  const handleTabKeyDown = (event: ReactKeyboardEvent<HTMLElement>) => { const backward = tabOrientation === "vertical" ? "ArrowUp" : "ArrowLeft"; const forward = tabOrientation === "vertical" ? "ArrowDown" : "ArrowRight"; if (![backward, forward, "Home", "End"].includes(event.key)) return; event.preventDefault(); const current = TABS.findIndex((tab) => tab.id === activeTab); const nextIndex = event.key === "Home" ? 0 : event.key === "End" ? TABS.length - 1 : event.key === backward ? (current - 1 + TABS.length) % TABS.length : (current + 1) % TABS.length; const next = TABS[nextIndex]; selectTab(next.id); setTimeout(() => document.getElementById(`tab-${next.id}`)?.focus(), 0); };
...
function SettingCard({ id, title, description, provenance, children }: { id?: string; title: string; description: string; provenance: string; children: ReactNode }) { return <article id={id} className="setting-card" tabIndex={id ? -1 : undefined}><div><h2>{title}</h2><p>{description}</p><small>{provenance}</small></div>{children}</article>; }
