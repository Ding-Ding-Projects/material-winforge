...
export default function SiteShell({ assetBase }: { assetBase: string }) {
  ...
  const resetAllSettings = () => { const next: Preferences = { ...DEFAULTS, personalVocabulary: prefs.personalVocabulary, settingsOwnership: { schemaVersion: 1, global: { ...DEFAULT_SITE_SETTINGS }, projects: [], activeProjectId: null } }; setPrefs(next); appendSettingsHistory("global-reset", "Reset all site settings to shipped defaults", next); announce("Site preferences and project ownership reset. Personal vocabulary was preserved.", "success", "Settings reset"); };
