...
function normalizePreferences(value: unknown): Preferences | null {
  ...
  return { ...DEFAULTS, ...v, ...effective, schemaVersion: 1, pinnedTabs, tabOrder, personalVocabulary, logoPreset: v.logoPreset ?? "forge", customLogo: v.customLogo ?? null, settingsOwnership } as Preferences;
}
...
export default function SiteShell({ assetBase }: { assetBase: string }) {
  ...
  const moveTab = (tabId: TabId, direction: -1 | 1, returnFocus = true) => {
    const pinned = prefs.pinnedTabs.includes(tabId);
    ...
    announce(dual(`Tab moved ${tabOrientation === "vertical" ? direction < 0 ? "up" : "down" : direction < 0 ? "left" : "right"}.`, `分頁已向${tabOrientation === "vertical" ? direction < 0 ? "上" : "下" : direction < 0 ? "左" : "右"}移。`, language), "success", "Tab ordering");
    if (returnFocus) setTimeout(() => document.getElementById(`tab-${tabId}`)?.focus(), 0);
  };
  ...
  const handleTabKeyDown = (event: ReactKeyboardEvent<HTMLElement>) => { if ((event.target as HTMLElement).getAttribute("role") !== "tab") return; const backward = tabOrientation === "vertical" ? "ArrowUp" : "ArrowLeft"; const forward = tabOrientation === "vertical" ? "ArrowDown" : "ArrowRight"; if (![backward, forward, "Home", "End"].includes(event.key)) return; event.preventDefault(); const current = orderedTabs.findIndex((tab) => tab.id === activeTab); const nextIndex = event.key === "Home" ? 0 : event.key === "End" ? orderedTabs.length - 1 : event.key === backward ? (current - 1 + orderedTabs.length) % orderedTabs.length : (current + 1) % orderedTabs.length; const next = orderedTabs[nextIndex]; selectTab(next.id); setTimeout(() => document.getElementById(`tab-${next.id}`)?.focus(), 0); };
