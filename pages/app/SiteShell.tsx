...
export default function SiteShell({ assetBase }: { assetBase: string }) {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") { setPaletteOpen(false); setPaletteBuilderOpen(false); setBuilderOpen(false); setNotificationOpen(false); setNotificationBuilderOpen(false); setSettingsHistoryOpen(false); setSettingsHistoryBuilderOpen(false); setSettingsRestoreId(null); }
    };
  ...
  const resetAllSettings = () => { const next: Preferences = { ...DEFAULTS, settingsOwnership: { schemaVersion: 1, global: { ...DEFAULT_SITE_SETTINGS }, projects: [], activeProjectId: null } }; setPrefs(next); appendSettingsHistory("global-reset", "Reset all site settings to shipped defaults", next); setVocabStatus("no-file"); setVocabMessage(""); announce("Site preferences reset."); };
