...
| Tab overflow, reorder, pin, groups, four searches, bulk close | Not proven | Measured overflow for the six existing tabs with a bounded all-tabs list, plain-default local search, and its own anchored full regex builder; reorder, pin, groups, bulk close, and remaining discovery scopes are missing | `docs/site/tab-overflow.md`, `docs/site/preferences.md` | Overflow trigger, search, result, current-tab, error, and empty-state copy English/Cantonese/bilingual | Unverified | Unverified | Missing | Incomplete |
