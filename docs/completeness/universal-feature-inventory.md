...
| Tab overflow, reorder, pin, groups, four searches, bulk close | Not proven | Measured ordinary-tab overflow, bounded all-tabs search with anchored full regex builder, validated persistent pin list, keyboard-reachable per-tab and overflow pin actions, stable pinned region, palette action, clear state labels, and explicit future bulk-close protection; reorder, groups, bulk close, and remaining discovery scopes are missing | `docs/site/tab-overflow.md`, `docs/site/tab-pinning.md`, `docs/site/preferences.md` | Overflow and pin trigger, action, state, result, protection, error, and empty-state copy English/Cantonese/bilingual | Unverified | Unverified | Missing | Incomplete |
