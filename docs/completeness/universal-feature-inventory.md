...
| Global defaults and per-project settings overrides | Explicit five-field desktop allowlist (`theme`, `lang`, `funnyEn`, `funnyZh`, `showEmojis`); other persisted records excluded by role | Missing | `docs/application/project-settings-overrides.md` | English only | Unverified | Partial desktop interaction evidence | Missing | Incomplete |
