...
Registration accepts a TOTP `otpauth://` URI or Base32 secret, with issuer/account metadata and bounded algorithm, digit, and period choices. Secret material is excluded from ordinary exports and history records. The registration state includes a local in-process QR rendering intent with a copyable URI text alternative; the desktop design remains a reference and does not claim an operating-system credential vault or runtime implementation.
